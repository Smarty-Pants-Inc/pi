...
async function generateModels() {
	...
	const omitted = generatorOptions.omitProviderIds;
	const retainedModels = filterOmittedProviders(allModels, omitted);
	...
	for (const model of filterOmittedProviders(openRouterCatalog.images, omitted)) {
		applyImageInputMetadata(model);
		providers[model.provider] ??= { chat: {}, image: {}, classifier: {} };
	...
	for (const model of filterOmittedProviders(classifierModels, omitted)) {
		providers[model.provider] ??= { chat: {}, image: {}, classifier: {} };
