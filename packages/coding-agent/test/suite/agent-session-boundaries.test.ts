...
describe("durable length recovery", () => {
	...
	it("omits a recoverable projected replacement by its source entry ID", async () => {
		...
		// smarty-dev#217: cancelled pre-prompt compaction retains input without dispatch.
		await expect(harness.session.prompt("next prompt")).rejects.toThrow("compaction was cancelled");
		expect(harness.session.getSteeringMessages()).toEqual(["next prompt"]);
		expect(harness.faux.state.callCount).toBe(0);
