...
describe("ordinary extension user append receipts", () => {
	...
	it("returns this first user's file receipt before provider completion, not a foreign equal-text entry", async () => {
		...
		try {
			...
			expect(entries.find((entry) => entry.id === receipt.entryId)).toEqual(
				sessionManager.getEntry(receipt.entryId),
			);
	...
	it.each(["input", "message_end"] as const)(
		"marks %s transformations rather than claiming exact original content",
		async (stage) => {
			let api!: ExtensionAPI;
			const harness = await createHarness({
				sessionManager: (directory) => SessionManager.create(directory, directory),
						...
						if (stage === "input") pi.on("input", () => ({ action: "transform", text: "changed" }));
							...
							pi.on("message_end", (event) => {
								if (event.message.role === "user")
									return { message: { ...event.message, content: [{ type: "text", text: "changed" }] } };
							});
					},
				],
			});
			...
			if (receipt.status !== "appended") throw new Error("Expected append");
			expect(harness.sessionManager.getEntry(receipt.entryId)).toMatchObject({
	...
	it.skipIf(process.platform === "win32" || process.getuid?.() === 0)(
		"returns not_written on real EACCES, with no provider call or indexed user",
		async () => {
			let api!: ExtensionAPI;
			const harness = await createHarness({
	...
	it.each(["steer", "followUp"] as const)(
		"correlates delivered %s input through the real native queue",
		async (deliverAs) => {
			let api!: ExtensionAPI;
			const harness = await createHarness({
				sessionManager: (directory) => SessionManager.create(directory, directory),
