...
describe("memory-only launch compaction override", () => {
	it.each(["stop", "error", "aborted"] as const)("prompts past retained %s usage without a preflight summarizer or history rewrite", async (stopReason) => {
		...
		try {
			...
			for (let i = 0; i < 6; i++) {
				retained.appendMessage({ role: "user", content: `retained ${i}: ${"old context ".repeat(512)}`, timestamp: Date.now() - 2000 });
				retained.appendMessage({
					...fauxAssistantMessage(`retained answer ${i}`, { stopReason, errorMessage: stopReason === "error" ? "maximum context length exceeded" : undefined }),
			...
			h.session.dispose();
			h.session.agent.replaceMessages(reopened.buildSessionContext().messages);
			session = new AgentSession({ agent: h.session.agent, sessionManager: reopened, settingsManager: settings,
				cwd, resourceLoader, modelRuntime, baseToolsOverride: {} });
			const events: AgentSessionEvent[] = [];
			session.subscribe(event => events.push(event));
			let requests = 0;
			h.setResponses([(_model, context) => {
				requests++;
				// A preflight summary request cannot contain this not-yet-inserted prompt.
				expect(context.messages.some(message => message.role === "user" && getMessageText(message) === "CURRENT ACTIVATION PROMPT")).toBe(true);
				return fauxAssistantMessage("activation ready");
			}]);
			await session.prompt("CURRENT ACTIVATION PROMPT");
			await settings.flush();
			expect(requests).toBe(1);
			expect(events.filter(event => event.type === "compaction_start" || event.type === "summarization_retry_attempt_start")).toEqual([]);
