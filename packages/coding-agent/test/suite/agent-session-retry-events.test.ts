	...
	describe.each(["Selected model is at capacity. Please try a different model.", "slow_down"])(
		...
		(errorMessage) => {
			it("recovers on the same model and retains the failed attempt in history", async () => {
				...
				expect(harness.eventsOfType("auto_retry_start")).toMatchObject([{ attempt: 1, maxAttempts: 2, errorMessage }]);
			...
			it.each([
				...
				harnesses.push(harness);
				harness.setResponses([fauxAssistantMessage("", { stopReason: "error", errorMessage: prefix + errorMessage })]);
			...
			it("cancels held retry backoff without another attempt", async () => {
				const harness = await createHarness({ settings: { retry: { enabled: true, maxRetries: 2, baseDelayMs: 100 } } });
				harnesses.push(harness);
			...
			it("retains a completed tool result and admits backoff steering exactly once", async () => {
				...
				expect(requests[1].messages.filter((message) => message.role === "user").map(getMessageText)).toEqual(["test"]);
				expect(requests[2].messages.filter((message) => message.role === "user").map(getMessageText)).toEqual([
					"test",
					"keep the completed tool result",
				]);
				expect(
					harness.eventsOfType("message_end")
