...
beforeEach(() => {
	...
	ports.load.mockReturnValue({ implementation: { ...ref, path: "/original/addon.node" } });
});
...
test("original object is bound before its first raw point and retains exact pre-A witness", () => {
	...
	const ticket = beginOrdinaryClockOperation("original-window-end");
	const raw = ordinaryClock.monotonic();
	const observed = finishOrdinaryClockOperation(ticket);
	expect(raw).toBeGreaterThanOrEqual(initial.initial.local.monotonicMs);
	expect(observed.eventMeaning).toBe("original-window-end");
