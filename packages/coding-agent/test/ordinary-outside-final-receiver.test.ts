...
function fixture(
	...
	if (options.constraintsCase === "invalid-transport")
		constraints.transport = { records: 0, bytes: 65536 };
	...
	if (options.constraintsCase === "substituted-transport") finalPrelaunch.constraints.transport = { records: 6, bytes: 65536 };
	...
	const link = put(`${state}/management/operational-link-${authorizationId}.json`, { reservation, capture, prelaunchPolicy });
	const enforcement = opaque("enforcement");
	const rows = [...files]
		.filter(([path]) => path !== controller.path && !(options.routeCase === "missing-retained" && path === clockRoute.path))
...
test.each([
	...
	"original closed constraints refuse %s before child-root prebind",
	(constraintsCase) => {
...
test.each([
	...
	"selected clock-route correspondence refuses %s",
	(routeCase) => {
