...
import type { OwnedProcessPoll, OwnedProcessRequest, OwnerAdmission, OwnerHost } from "../src/core/owner-effects.ts";
...
const f = vi.hoisted(() => ({
	now: 0,
	seal: vi.fn<() => Promise<void>>(),
	quarantine: vi.fn(),
	release: vi.fn(),
	cancel: vi.fn(),
	stop: vi.fn(),
	poll: vi.fn<() => OwnedProcessPoll>(),
	retire: vi.fn(),
	complete: vi.fn(),
}));
...
vi.mock("../src/core/ordinary-clock.ts", () => ({
	// These budget schedules do not prepare clock evidence. Native completion
	// remains the original modeled f.complete; no obsolete callback enrollment.
	hasOriginalClockEvidence: () => false,
}));
vi.mock("../src/core/session-manager.ts", () => ({
	persistOwnedTerminalSession: async () => {},
	// biome-ignore lint/complexity/noStaticOnlyClass: Mock the static-only SessionManager surface.
	SessionManager: class {
		static inMemory() {
			return { getHeader: () => ({ id: "inert", timestamp: "2026-09-20T00:00:00Z" }) };
		}
		static openOwned() {
...
vi.mock("../src/core/owner-effects.ts", () => ({
	OwnedJournal: class {
		static acquire() {
			// biome-ignore lint/complexity/noThisInStatic: Mock OwnedJournal.acquire returning the mocked class instance.
			return new this();
		}
		inspectResources() {}
		inspectPermission() {}
		...
		terminal(invoke: () => unknown) {
			return invoke();
		}
		beginOperation() {
			return { check() {}, bindProcess() {}, complete: f.complete };
		}
		prepareProcess() {
...
const host = {
	profile: { limits: { closeTimeoutMs: 100, processTimeoutMs: 800, outputBytes: 1024 } },
} as unknown as OwnerHost;
...
describe("original close budget", () => {
	test("seal time is not followed by a fresh close interval", async () => {
		f.seal.mockImplementation(async () => {
			f.now = 100;
		});
		const value = owner();
...
test("native receipt data precedes release dispatch and one-use acceptance", () => {
	...
	expect(accept.indexOf("!lifecycle_live(task) || (successor && !lifecycle_live(successor))")).toBeGreaterThan(
		prepared,
	);
