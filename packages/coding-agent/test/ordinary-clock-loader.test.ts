...
import { OwnerHost, receiveOriginalClockSource } from "../src/core/owner-effects.ts";
...
const ports = vi.hoisted(() => ({
	...
	close: vi.fn(),
	profile: {
		runtime: { architecture: process.arch, kind: "node", version: process.versions.node },
		artifacts: {
...
test("artifact-only preparation precedes Host validation and later Host rejects a different module", () => {
	...
	const native = new Proxy(
		{ abi: 2, clockAbi: 1, readClockSample: ports.read, validateHost: ports.validate },
		{
			has: () => true,
