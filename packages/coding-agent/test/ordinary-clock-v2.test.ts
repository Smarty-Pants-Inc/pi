...
import {
	...
	parseSc085AuditClockQualification,
	type Sc085AuditClockQualificationV2,
} from "../src/core/ordinary-sc085-source/sc085-admission.ts";
...
const metadata: Sc085AuditClockQualificationV2 = {
	...
	owner: { ownerEpoch: "child", sessionId: "session", allocationId: "allocation" },
	admittedClockId: "parent",
	nativeIdentity: ref,
	...
	mapping: {
		...
		producer: ref,
		implementation: ref,
		uncertaintyNs: "1000000000",
	},
	validity: { notBeforeWallMs: 1, expiresWallMs: 2 },
};
...
test.each(["-1", "01", "1.1", "1000000001"])("v2 refuses malformed/over-budget error %s", (uncertaintyNs) => {
	expect(() =>
		canonicalSc085AuditClockQualification({ ...metadata, mapping: { ...metadata.mapping, uncertaintyNs } }),
	).toThrow();
});
test("v1 cannot be relabeled into v2", () => {
	const old = {
		...
		mapping: { kind: "same-original-source", nativeClockId: "local", uncertaintyMs: 0 },
	};
...
test("original routes use v2 carriers and the last existing recheck, no old float-copy emission", () => {
	const source = readFileSync(
		new URL("../src/core/ordinary-sc085-source/operational-admission.ts", import.meta.url),
		"utf8",
	);
