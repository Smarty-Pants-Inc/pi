...
function validateQualification(value: unknown): asserts value is Sc085AuditClockQualificationV2 {
	...
	assert(
		typeof value.mapping.uncertaintyNs === "string" &&
			/^(0|[1-9][0-9]{0,19})$/.test(value.mapping.uncertaintyNs) &&
			BigInt(value.mapping.uncertaintyNs) <= 1_000_000_000n,
...
export function receiveSc085AuditClockQualification(
	...
	for (const ref of [value.mapping.contract, value.mapping.basis, value.mapping.producer])
		sc085RetainedBytes(ref, retained);
