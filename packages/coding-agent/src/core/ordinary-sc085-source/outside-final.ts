...
export function createOperationalOutsideFinal(input: OriginalOutsideFinalSelection): OriginalOutsideFinalReceiver {
	...
	try {
		...
		assert.deepEqual(
			prelaunch,
			record(reservation.policy).operational_prelaunch,
		...
		for (const [key, value] of Object.entries(selected.authority.expected)) {
			...
			const pattern = ["repository_id", "run_id", "run_attempt"].includes(key)
				...
				: key === "recipe_sha256"
					? /^[a-f0-9]{64}$/
					: /^[a-f0-9]{40}$/;
			const actual = clockRoute.reservation[key];
		...
		assert.deepEqual(derivation.controllerSource, selected.authority.controller, "OPS_OUTSIDE_CLOCK_ROUTE_CONTROLLER");
		assert.deepEqual(clockOwner.controllerSource, selected.authority.controller, "OPS_OUTSIDE_CLOCK_ROUTE_CONTROLLER");
		...
		for (const key of ["records", "bytes"] as const) {
			for (const row of [clockRoute, ...stages]) {
				...
				assert(typeof value === "number" && Number.isSafeInteger(value) && value > 0, "OPS_OUTSIDE_CLOCK_ROUTE_BOUND");
			}
			assert.equal(
