...
import {
	...
	ordinaryClock,
	originalClockAssociation,
	originalClockInitialObservation,
...
import {
	type OperationalEnforcementMethod,
	operationalEnforcementMethodRefs,
	parseOperationalEnforcementMethod,
} from "./enforcement-method.ts";
...
import { selectOriginalHMeter } from "./h-meter-selection.ts";
import {
...
export function createOperationalAdmission(
	...
	const receiveParentClock = (
		clock: OriginalClockReceiving,
		witness: Parameters<typeof receiveClockParentAssociation>[0]["witness"],
	) => {
		assert(clock.received && originalInitial && originalAuthorization, "OPS_CLOCK_ORIGINAL_QUERY_REQUIRED");
		const associated = guarded(() =>
			receiveClockParentAssociation({
				binding: originalAuthorization!.authorization.operational_binding,
				initial: originalInitial!,
				retained: retained.retained,
				prepared: originalClockAssociation(),
				witness,
				guard: clock.received!.projection.reply.raw,
			}),
		);
		assert(
			associated.clockId === selection!.clock.admittedClockId &&
				Number(BigInt(associated.uncertaintyNs)) / 1_000_000 === selection!.uncertaintyMs,
			"OPS_CLOCK_ORIGINAL_POLICY_BUDGET",
		);
	...
	const receiveCompaction = () => {
		...
		return guarded(() =>
			receiveOriginalCompactionMethod({
	...
	sc085HMeters.set(result, (signal) => {
		recheck("preflight");
		return guarded(() => {
			assert(originalInitial, "OWNER_H_ORIGINAL_INITIAL_REQUIRED");
			return selectOriginalHMeter(originalInitial, retained.retained, signal);
		});
	});
	...
	sc085BootstrapSamplers.set(result, (recorder) => {
		try {
			...
			const observation =
				lastBootstrapMono === undefined
					? originalClockInitialObservation()
					: commitOrdinaryClockOperation(beginOrdinaryClockOperation("bootstrap-accounting"));
			...
			const clock: OriginalClockReceiving = {
				query: { beforeNs: observation.parent.before.monotonicNs, afterNs: observation.parent.after.monotonicNs },
				recorder,
			};
	...
	sc085Qualifications.set(result, (owner, selector) => {
		try {
			...
			const compaction =
				selector.kind === "compaction" ? OrdinaryOwnerContext.prototype.compactionReceipt.call(owner) : undefined;
			if (compaction && selector.kind === "compaction")
				assert(
					compaction.append === "confirmed" &&
						compaction.entryId === selector.entryId &&
						compaction.observation?.eventMeaning === "compaction-append",
					"OPS_COMPACTION_ORIGINAL_ENDPOINT",
				);
			const original = compaction
				? {
						scope: {
							ownerEpoch: compaction.ownerEpoch,
							sessionId: compaction.sessionId,
							allocationId: compaction.allocationId,
						},
						clockIdentity: owner.operationalAudit.clockIdentity,
						stamp: {
							monotonicMs: compaction.observation!.local.monotonicMs,
							wallMs: compaction.observation!.local.wallMs,
							clockId: owner.operationalAudit.clockIdentity.id,
							uncertaintyMs: null,
							parent: compaction.observation!.parent,
							clockSequence: compaction.observation!.sequence,
			...
			const clock: OriginalClockReceiving = {
				query: { beforeNs: stamp.parent.before.monotonicNs, afterNs: stamp.parent.after.monotonicNs },
				recorder: store,
			};
		...
		try {
			...
			const qualification: Sc085AuditClockQualificationV2 = {
				...
				mapping: {
					...
					nativeClockId: audit.id,
					contract: parentClock.contract,
					basis: parentClock.basis,
					producer: prepared.producer,
					implementation: prepared.implementation,
					uncertaintyNs: parentClock.uncertaintyNs,
				},
...
const sc085HMeters = new WeakMap<object, (signal: AbortSignal) => OriginalHMeterSelection>();
/** Original bootstrap only, before allocation; no SDK/ordinary export. */
export function receiveSc085OriginalHMeter(receiving: Sc085OriginalReceiving): OriginalHMeterSelection {
	return withSc085Route(receiving, (route) => {
		assert(route.phase === "entered" && !route.hReceived, "OWNER_H_ORIGINAL_RECEIVING_ONCE");
		route.hReceived = true;
		assert(route.signal, "OWNER_H_ORIGINAL_SIGNAL_REQUIRED");
		route.signal.throwIfAborted();
		return sc085HMeters.get(route.supplier)!(route.signal);
	});
}
...
export function prepareSc085OriginalReceiving(
	...
	return withSc085Supplier(supplier, () => {
		...
		originalReceivings.set(receiving, { supplier, recorder, phase: "prepared", ...(signal ? { signal } : {}) });
