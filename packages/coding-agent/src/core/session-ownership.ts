...
export class SessionOwnership {
	...
	acceptOriginalHMeter(): void {
		this.assertActive();
		if (!this.#allocated) throw new Error("OWNER_H_ORIGINAL_ALLOCATION_REQUIRED");
		this.#journal.acceptOriginalHMeter();
	}

	borrowOriginalHMeter() {
		this.assertActive();
		if (!this.#allocated) throw new Error("OWNER_H_ORIGINAL_ALLOCATION_REQUIRED");
		return this.#journal.borrowOriginalHMeter();
	}
	...
	async effect<T>(
		...
		if ("completed" in options) throw new Error("OWNER_COMPLETION_CALLBACK_UNSUPPORTED");
		return (await this.#effect(request, action, options.signal, false)).value;
	}

	/** The same effect lifecycle, with DATA from its actual receipt completion.
	 * No synchronous completion observer or acknowledgement supplier is accepted. */
	async effectWithCompletion<T>(
		request: OwnedEffectRequest,
		action: (effect: OwnedEffect) => Promise<T> | T,
		options: { signal?: AbortSignal } = {},
	): Promise<{ value: T; completion?: OriginalClockObservation }> {
		if ("completed" in options) throw new Error("OWNER_COMPLETION_CALLBACK_UNSUPPORTED");
		return this.#effect(request, action, options.signal, hasOriginalClockEvidence());
	}
	...
	async #effect<T>(
		...
		try {
			completion = await operation?.complete(errors.length > 0, observeCompletion && !errors.length);
		} catch (error) {
			errors.push(error);
			this.#phase = "quarantined";
			try {
			...
			// No new completion/persistence attempt. The native lifecycle retains
			// and completes its original receipt once even on a before-edge failure.
		}
		...
		return { value: result as T, completion };
	}
	...
	async #close(hooks: {
		...
		try {
			...
			const failure =
				errors.length && !errors.includes(error) && error !== closeFailure
					? new AggregateError([...errors, error], "OWNER_CLOSE_FAILED", { cause: errors[0] })
					: error;
			this.#phase = "quarantined";
			this.#allowTerminal();
...
Object.freeze(SessionOwnership.prototype);
Object.freeze(SessionOwnership);
