...
import type { NativeClockSample, OriginalClockObservation } from "./ordinary-clock-evidence.ts";
import {
...
import {
	...
	OriginalHMeter,
	type OriginalHMeterBorrow,
	type OriginalHMeterSelection,
} from "./owner-h-meter.ts";
...
const ownerHosts = new WeakMap<
	OwnerHost,
	{
		...
		digest: string;
		hMeter?: OriginalHMeter;
		hMeterReceiving?: boolean;
	}
>();

function hMeterBinding(native: NativeOwnerBinding): NativeHMeterBinding {
	const port = native as NativeOwnerBinding & Partial<NativeHMeterBinding>;
	if (port.hMeterAbi !== 1) throw new Error("OWNER_H_NATIVE_ABI");
	for (const name of [
		"startOriginalHMeter",
		"pollOriginalHMeter",
		"cancelOriginalHMeter",
		"acceptOriginalHMeter",
		"borrowOriginalHMeter",
		"returnOriginalHMeter",
		"finishOriginalHMeter",
	] as const)
		if (typeof port[name] !== "function") throw new Error("OWNER_H_NATIVE_ABI");
	return port as NativeOwnerBinding & NativeHMeterBinding;
}
...
async function settleNativeLifecycle<T>(
	...
	for (;;) {
		...
		try {
			const ticket = observation ? beginOrdinaryClockOperation("native-operation-completion") : undefined;
			completed = true;
			result = receipt.complete();
			if (ticket) observation!.current = commitOrdinaryClockOperation(ticket);
		} catch (cause) {
			let error = cause;
			if (observation) {
				try {
					failOrdinaryClockOperation(cause);
				} catch (first) {
					error = first;
				}
			}
...
/** Fixed original native sample source. No caller can supply a read function. */
export function readOriginalPreparedClockSample(): NativeClockSample {
	const native = preparedClockModule?.native;
	if (!native || native.clockAbi !== 1 || typeof native.readClockSample !== "function")
		throw new Error("OWNER_CLOCK_NOT_PREPARED");
	return native.readClockSample();
}
...
class OwnedOperation {
	...
	async complete(failed: boolean, observe = false): Promise<OriginalClockObservation | undefined> {
		if (typeof observe !== "boolean") throw new Error("OWNER_COMPLETION_CALLBACK_UNSUPPORTED");
		const observation: NativeCompletionObservation | undefined = observe ? {} : undefined;
		await this.#lifecycle(() => this.#native.completeOperation(this.#handle, failed), observation);
...
class OwnedProcess {
	...
	constructor(key: symbol, native: NativeOwnerBinding, handle: NativeLaunch, lifecycle: NativeLifecycleQueue) {
		if (key !== constructionKey) throw new Error("OWNER_NATIVE_CONSTRUCTION");
		this.#native = native;
		this.#handle = handle;
		this.#lifecycle = lifecycle;
...
export class OwnedJournal {
	...
	/** The core calls only AFTER original context/Source binding. Native checks
	 * this exact allocated lease against the separately received H selection. */
	acceptOriginalHMeter(): void {
		this.assertActive();
		const meter = ownerHosts.get(this.#host)?.hMeter;
		if (!this.#activated || !meter) throw new Error("OWNER_H_ORIGINAL_HOST_REQUIRED");
		meter.accept(this.#lease);
	}

	borrowOriginalHMeter(): OriginalHMeterBorrow {
		this.assertActive();
		const meter = ownerHosts.get(this.#host)?.hMeter;
		if (!this.#activated || !meter) throw new Error("OWNER_H_ORIGINAL_HOST_REQUIRED");
		return meter.borrow(this.#lease);
	}
	...
	beginOperation(request: OwnedEffectRequest): OwnedOperation {
		...
		return new OwnedOperation(
			constructionKey,
			this.#native,
			this.#native.beginOperation(this.#lease, request),
			(start, observation) => this.#lifecycle(start, observation),
		);
	}
	...
	prepareProcess(request: OwnedProcessRequest, timeoutMs: number): OwnedProcess {
		...
		return new OwnedProcess(
			constructionKey,
			this.#native,
			this.#native.prepareLaunch(this.#lease, request, timeoutMs),
			(start) => this.#lifecycle(start),
		);
	}

	#lifecycle<T>(start: () => NativeLifecycleReceipt<T>, observation?: NativeCompletionObservation): Promise<T> {
		const pending = this.#lifecycleTail.then(() => {
			if (this.#quarantined || this.#released)
				throw new Error("OWNER_LIFECYCLE_UNKNOWN", { cause: this.#quarantineFailure?.cause });
			return settleNativeLifecycle(start(), () => this.#native.cancelLifecycle(this.#lease), observation);
		});
		...
		void pending.catch((error: unknown) => {
			this.#quarantineFailure ??= { cause: error };
			this.#quarantined = true;
		});
		return pending;
	}
	...
	quarantine(): void {
		...
		if (pending) {
			...
			void settleNativeLifecycle(pending, () => this.#native.cancelLifecycle(this.#lease)).catch(
				(error: unknown) => {
					this.#quarantineFailure ??= { cause: error };
				},
			);
	...
	async release(): Promise<void> {
		if (!this.#sealed || this.#quarantined || this.#releaseAttempted)
			throw new Error("OWNER_NOT_RETIRED", { cause: this.#quarantineFailure?.cause });
...
export class OwnerHost {
	...
	/** Reachable only from the original async core construction, never a path
	 * reopen/getter or replacement for the effect cgroup root. */
	async receiveOriginalHMeter(selection: OriginalHMeterSelection): Promise<void> {
		const received = ownerHosts.get(this);
		if (!received || received.hMeterReceiving) throw new Error("OWNER_H_ORIGINAL_HOST_ONCE");
		received.hMeterReceiving = true;
		received.hMeter = await OriginalHMeter.receive(hMeterBinding(received.native), received.handle, selection);
	}

	joinOriginalHMeter(): Promise<void> {
		const received = ownerHosts.get(this);
		if (!received) throw new Error("OWNER_NATIVE_HOST_REQUIRED");
		return received.hMeter?.joinBorrow() ?? Promise.resolve();
	}

	finishOriginalHMeter(): void {
		const received = ownerHosts.get(this);
		if (!received) throw new Error("OWNER_NATIVE_HOST_REQUIRED");
		received.hMeter?.finish();
	}
	...
	close(): void {
		const received = ownerHosts.get(this);
		if (!received) throw new Error("OWNER_NATIVE_HOST_REQUIRED");
		received.hMeter?.cancelUnaccepted();
...
Object.freeze(OwnedOperation.prototype);
Object.freeze(OwnedOperation);
Object.freeze(OwnedJournal.prototype);
Object.freeze(OwnedJournal);
Object.freeze(OwnerHost.prototype);
Object.freeze(OwnerHost);
