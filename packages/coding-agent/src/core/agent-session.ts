...
import type { OriginalCompactionAttempt } from "./ordinary-compaction.ts";
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#ordinaryOwner?.bindSessionAdmission(
			this,
			(recheck, enroll) => this.#requestOrdinaryWake(recheck, enroll),
			(attempt) => this.#compactOriginal(attempt),
		);
	...
	private async _persistAgentEvent(event: AgentEvent): Promise<void> {
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "custom") {
				...
				const entryId = this.#ordinaryOwner
	...
	private _flushPendingCustomMessagesOwnedTerminal(): Promise<void> {
		return this.#queuePendingFlush(async () => {
			while (this._pendingCustomMessages.length) {
				...
				const id = await appendOwnedTerminalCustomMessage(this.sessionManager,
	...
	async #compactSession(
		...
		try {
			...
			const append = () => attempt
				? appendOriginalCompaction.call(this.sessionManager, summary, firstKeptEntryId, tokensBefore, details, false, usage)
				: this.sessionManager.appendCompaction(summary, firstKeptEntryId, tokensBefore, details, fromExtension, usage);
			const compactionId = attempt
				? attempt.append({ summary, firstKeptEntryId, tokensBefore, details, usage }, this.sessionManager.getLeafId(), append)
				: append();
	...
	async #compactOriginal(attempt: OriginalCompactionAttempt): Promise<CompactionResult> {
		...
		if (this.#ordinaryPreflights || !this.isIdle || this.isRetrying || this.isBashRunning ||
			originalAgentSignal.call(this.#originalAgent) || this.#originalAgent.hasQueuedMessages() ||
			this.pendingMessageCount || (this._extensionMode === "tui" && !this.#pendingModeInput) ||
			this.#pendingModeInput?.()) throw new Error("OPS_COMPACTION_SESSION_NOT_IDLE");
		owner.assertSessionStart(this);
		const model = this.model;
		const stream = this.agent.streamFunction;
		const branchIdentity = () => JSON.stringify(this.sessionManager.getBranch().filter((entry) =>
			!(entry.type === "custom" && ["smarty-sense:count-reservation-v1", "smarty-sense:count-qualification-v1",
				"smarty-sense:provider-reservation-v1", "smarty-sense:provider-usage-v1"].includes(entry.customType))));
		let branch = branchIdentity();
		const checkState = () => {
			owner.assertSessionStart(this);
			this._compactionAbortController?.signal.throwIfAborted();
			if (this.model !== model || this.agent.streamFunction !== stream || this.#ordinaryPreflights ||
				this._isAgentRunActive || this.isRetrying || this.isBashRunning ||
				this.#originalAgent.hasQueuedMessages() || this.pendingMessageCount || this.#pendingModeInput?.() ||
				branchIdentity() !== branch)
				throw new Error("OPS_COMPACTION_SESSION_CHANGED");
		};
		attempt.fence(checkState);
		return this.#compactSession(undefined, {
			...attempt,
			append: (result, beforeLeaf, invoke) => {
				attempt.check();
				return attempt.append(result, beforeLeaf, invoke, () => { branch = branchIdentity(); });
			},
		});
	}
	...
	async #requestOrdinaryWake(
		...
		const ready = () => {
			try { owner.assertCompactionIdle(); } catch { return false; }
			owner.assertSessionStart(this);
