	...
	const methods = Object.freeze({
		async hold(selected: SetupRawRef, segment: "rapid" | "failures") {
			try {
				...
				state = {
					...
					deadline:
						audit.observeSince(cursor).start.monotonicMs + admission.validity.maxHoldMs - 2 * start.uncertaintyMs,
				};
		...
		async release(token: object, checkpoint: OrdinaryObservedRefresh) {
			try {
				...
				const final = audit.validateSc085Checkpoint(checkpoint, context);
				assert(
			...
			try {
				...
				for (;;) {
					...
					for (const [qualified, originalStamp] of [
						...
						assert.deepEqual(
							(original as Record<string, unknown>).stamp,
							originalStamp,
							"OWNER_SC085_SEAL_ORIGINAL",
						);
						assert(
							originalStamp.parent && originalStamp.clockSequence && originalStamp.eventMeaning,
							"OWNER_SC085_SEAL_WITNESS",
						);
					...
					const elapsedNs =
						endNs - BigInt(exposure.monotonicNs) - BigInt(exposure.uncertaintyNs) - BigInt(end.uncertaintyNs);
					assert(
					...
					if (elapsedNs < requiredNs) {
						assert(wakes < 2 && (wakes === 0 || endNs > previousObserved), "OWNER_SC085_OBSERVATION_NO_PROGRESS");
