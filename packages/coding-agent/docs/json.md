...
{"type":"agent_settled","outcome":"completed"}
```
...
| `agent_settled` | `outcome` | Pi will not continue automatically through retries, compaction recovery, or queued messages. `outcome` is `completed`, `aborted` (cancelled by a user abort, an extension that cancelled compaction, or shutdown), or `error` (provider, run, or compaction failure). |
...
```json
{"type":"summarization_retry_scheduled","attempt":1,"maxAttempts":1,"delayMs":2000,"errorMessage":"terminated"}
...
Compaction has one retry shared across history and turn-prefix summaries, with a 20-minute deadline for the complete operation. A deadline failure is reported as failed, not as a user abort. Branch summaries and ordinary assistant turns retain their configured retry policy.
