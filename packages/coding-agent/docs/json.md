...
| `agent_settled` | `outcome` | Pi will not continue automatically through retries, compaction recovery, or queued messages. `outcome` is `completed`, `aborted` (user abort, also during retry backoff or `agent_before_settle`), or `error`. |
