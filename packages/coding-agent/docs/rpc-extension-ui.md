...
Prompt the user to choose from a list. Every dialog request includes its effective timeout in milliseconds; the agent auto-resolves with `undefined` if the client does not respond in time.
