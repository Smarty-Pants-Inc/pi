...
If recovery compaction fails or is cancelled, Pi keeps the omission edits, appends no compaction, and schedules no internal retry. Queued work stays queued, and the run settles without calling `agent_before_settle` to request further work. After successful recovery, later boundary hooks see the repaired projection. Raw transcript history, exports, billing totals, and history-search extensions can still inspect the omitted attempt.
